...
class gltfAudioSource extends GltfObject
{
    ...
    async setAudioFromUri()
    {
        ...
        {
            this.decodedAudio = await gltfAudioSource.loadHTMLAudio(this.uri).catch( (error) => {
                console.error(error);
            });
        }
        else
    ...
    async setAudioFromBufferView(gltf)
    {
        ...
        if(typeof(AudioContext) !== 'undefined' && this.mimeType === "audio/mpeg")
        {
            ...
            this.decodedAudio = await gltfAudioSource.loadHTMLAudio(objectURL).catch( () => {
    ...
    async setAudioFromFiles(files)
    {
        ...
        if (typeof(AudioContext) !== 'undefined' && (this.mimeType === "audio/mpeg"))
        {
            ...
            this.decodedAudio = await gltfAudioSource.loadHTMLAudio(audioData).catch( () => {
